...
- UI: 현금 유입 분석 캐러셀의 좌우 화살표 동작을 사용자 기대 방향에 맞게 교정함.
- 동작 변경: 왼쪽 화살표 클릭 시 이전 2주 기간으로 이동, 오른쪽 화살표 클릭 시 다음 2주 기간으로 이동하도록 수정.
- 구현 방식: 캐러셀 기본 이전/다음 버튼 대신 커스텀 좌우 버튼을 사용해 기간 이동 액션을 명시적으로 매핑하고, 각 방향별 이동 가능 여부에 따라 비활성화 처리.
- 적용 파일: `components/dashboard/CashFlowAnalysis.tsx`
- 범위: 프론트엔드 캐러셀 내비게이션 UX 수정만 수행, DB/API/인증 로직 변경 없음.
