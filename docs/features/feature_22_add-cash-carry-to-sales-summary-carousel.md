...
- UI: Sales Summary 캐러셀에 `Cash & Carry` KPI 카드가 추가되어 총 5개 카드가 표시된다.
- UI: Cash & Carry 카드는 기존 KPI 카드와 동일한 스타일과 전일 대비 증감률 UI를 사용한다.
- UI: Cash & Carry 값은 일일 매출 입력 데이터의 `cashAndCarrySales` 필드를 그대로 사용한다.
