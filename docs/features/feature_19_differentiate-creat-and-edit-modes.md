# [Feature #19] Differentiate Creat and Edit modes
...
- 기존 매출 데이터가 있는 날짜는 '저장하기' 대신 '수정하기' 버튼 표시
- 저장된 데이터는 드로어 오픈 시 자동 조회하여 필드에 표시
- 수정 후 '수정하기' 버튼 클릭 시 기존 데이터 업데이트 처리
- 신규 데이터가 없는 날짜는 기존과 동일하게 '저장하기' 버튼 표시

## 2. 반영된 최종 스펙 (AI 작업 결과물)
<!-- 코드 작업이 끝나면 변경된 스펙(DB/API/UI)을 여기에 요약해. -->

- UI: Daily Sales 드로어는 열릴 때 선택된 지점/날짜 기준 기존 매출 데이터를 자동 조회하고, 데이터 존재 시 입력 필드를 기존 값으로 채운다.
- UI: 기존 데이터가 조회되면 제출 버튼 문구는 `수정하기`로, 신규 데이터가 없으면 기존처럼 `저장하기`로 표시된다.
- API: 제출 동작은 기존 `POST /api/v1/sales` 업서트 계약을 그대로 사용하며, 편집 모드에서도 동일 엔드포인트로 기존 레코드를 업데이트한다.
