...
- API/집계 로직: 일별(Daily) 트렌드 기준을 최근 7일 고정에서 현재일이 속한 주의 월요일~현재일 범위로 변경.
- 표시 규칙: 미래 날짜(현재일 이후)는 포함하지 않으며, 월요일부터 현재일까지의 날짜만 라벨로 노출.
- 데이터 처리: 월~현재일 범위 내에서 매출 데이터가 없는 날짜는 0으로 보정하여 그래프 흐름이 끊기지 않도록 처리.
- 예시 동작: 오늘이 목요일이면 월~목까지만 표시, 오늘이 일요일이면 월~일 전체 표시.
- 적용 파일: `lib/db-queries.ts`
- 범위: 대시보드 트렌드 집계 로직 변경만 수행, DB 스키마/API 계약/인증 로직 변경 없음.
