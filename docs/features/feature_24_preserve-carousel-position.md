# [Feature #24] Preserve carousel position

---
> 🤖 **Cursor Agent 작업 지시서 (System Prompt)**
> 너는 에이전시의 수석 개발자다. 주어진 마스터 문서(@01_db_schema.md, @02_api_routes.md, @03_frontend_ui.md)는 시스템 뼈대 파악을 위해 반드시 읽되, 절대 직접 수정하지 마라.
> 
> **[작업 3단계 프로세스 - 엄수할 것]**
> 1. **작업 중**: 이 Feature MD의 요구사항과 마스터 MD(참조용)를 보고 코드를 짠다. 스택은 [Next.js + Tailwind + Neon/Supabase Auth]로 고정한다.
> 2. **작업 완료 직후**: 마스터 문서는 건드리지 말고, 이 Feature MD 하단 [반영된 최종 스펙]에 변경된 스펙만 요약 기록해라.
> 3. **다음 작업 시작 전 (역동기화 준비)**: 개발자가 별도의 '역동기화 프롬프트'를 내리기 전까지 마스터 문서는 현재 상태를 유지한다.
---

## 1. 요구사항 명세 (Issue Content)
## Background
- 매출 요약 캐러셀에서 이전 날짜 매출을 확인 중 Gross / Net 토글 시 캐러셀이 오늘 날짜로 초기화됨.
- 토글은 표시되는 데이터만 변경되어야 하며, 현재 보고 있는 날짜는 유지되어야 함.

## Requirements
- Gross → Net 또는 Net → Gross 토글 시 현재 캐러셀 위치 유지할 것.
- 사용자가 보고 있는 날짜의 매출 요약을 그대로 유지한 상태에서 데이터만 변경할 것.
- 캐러셀이 오늘 날짜로 초기화되지 않도록 수정할 것.
- 좌우 이동 상태 및 선택된 날짜 정보가 토글 후에도 유지될 것.

## Expected Result
- 사용자가 이전 날짜를 보고 있다면 토글 후에도 동일한 날짜에 머무름.
- Gross / Net 전환 시 데이터만 변경되고 캐러셀 위치는 변경되지 않음.
- 불필요한 캐러셀 초기화가 제거되어 사용자 경험 향상됨.

## 2. 반영된 최종 스펙 (AI 작업 결과물)
<!-- 코드 작업이 끝나면 변경된 스펙(DB/API/UI)을 여기에 요약해. -->

- DB: 변경 없음.
- API: 변경 없음.
- UI:
	- `Sales Summary` 캐러셀의 슬라이드 갱신 시, 기존처럼 마지막(오늘 날짜)으로 강제 이동하지 않도록 로직 수정.
	- 현재 사용자가 보고 있던 슬라이드의 `date`를 기준으로 새 데이터에서도 동일 날짜 인덱스를 찾아 복원하도록 변경.
	- 동일 날짜가 없는 경우에만 마지막 슬라이드로 fallback 하도록 처리.
	- Gross/Net 토글 시 캐러셀 좌우 이동 상태 및 선택 날짜를 유지한 채 카드 값만 갱신되도록 개선.

---
### ⚠️ 개발자 전용: 역동기화 프롬프트 템플릿 (복사해서 사용)
아래 프롬프트를 복사하여 새 채팅창에 입력하고 마스터 문서를 최신화하세요.
```text
@01_db_schema.md @02_api_routes.md @03_frontend_ui.md @(현재 작업한 코드 폴더/파일들)
현재 구현된 실제 소스 코드를 정답으로 간주하고, 변경된 사항을 바탕으로 마스터 MD 파일 3개를 최신화(덮어쓰기)해. 코드는 절대 수정하지 마.
```
