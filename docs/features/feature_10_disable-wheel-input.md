...
- UI: Daily Sales 입력 모달의 모든 숫자 입력 필드(Card, Cash, Uber Eats, DoorDash, Cash & Carry, Tips, Actual Closing Cash)에 마우스 휠 입력 무효화 처리 적용.
- 동작: 입력 필드가 포커스된 상태에서 휠 스크롤이 발생하면 필드 포커스를 해제하여 브라우저의 숫자 자동 증감 동작이 발생하지 않도록 수정.
- 결과: 값 변경은 직접 타이핑/삭제/붙여넣기 검증을 통과한 입력으로만 가능하며, 페이지 스크롤 중 의도치 않은 값 변경을 방지.
- 적용 파일: `components/forms/DailySalesForm.tsx`
- 범위: 프론트엔드 입력 UX 안정성 개선만 수행, DB/API/인증 로직 변경 없음.
