...
현재는 일일 매출 입력 시 날짜를 직접 선택해야 하는데, 일일 매출 입력 화면 진입 시 오늘 날짜가 기본값으로 자동 선택되도록 수정해줘.
다른 날짜의 매출을 입력하거나 수정해야 하는 경우에는 기존과 동일하게 달력을 통해 날짜를 변경할 수 있게 해줘
...
- UI: 일일 매출 입력 드로어 진입 시 날짜 필드의 기본값이 오늘 날짜로 자동 선택되도록 반영.
- UI: 드로어 재진입 시 날짜 기본값은 오늘로 유지되며, 오늘 날짜에 이미 저장된 매출이 있으면 해당 값들을 자동으로 폼에 채워서 이어서 수정 가능하도록 반영.
- UI: 사용자가 기존과 동일하게 캘린더에서 다른 날짜를 자유롭게 선택/변경 가능.
- DB/API: 변경 없음.
